...
export const columns: ColumnDef<MeetingGetMany[number]>[] = [
  {
    accessorKey: "name",
    header: () => (
      <span className="pl-2 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">
        Session Details
      </span>
    ),
    cell: ({ row }) => (
      <div className="flex flex-col gap-y-1.5 py-2 pl-2">
        <span className="text-sm font-bold tracking-tight truncate max-w-[300px] capitalize">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const status =
        (row.original.status || "upcoming") as keyof typeof statusConfig;
      ...
      return (
        <Badge
          variant="outline"
          className={cn(
            "h-7 rounded-lg border px-2.5 font-bold text-[10px] uppercase flex items-center gap-x-1.5",
            config.className
          )}
        >
          <Icon
  ...
  },
  {
    ...
    header: () => (
      <span className="font-bold uppercase tracking-wider text-[10px] text-muted-foreground hidden md:inline">
        Time Log
      </span>
    ),
    cell: ({ row }) => {
      ...
      return (
        <div className="hidden md:flex items-center gap-x-3">
          <Badge
            variant="secondary"
            className="h-8 rounded-xl bg-muted/50 border px-3 font-mono text-xs flex items-center gap-x-2"
          >
          ...
          {duration && (
            <div className="w-12 h-1 bg-muted rounded-full overflow-hidden hidden lg:block">
              <div
                ...
                style={{
                  width: `${Math.min((duration / 3600) * 100, 100)}%`,
                }}
              />
            </div>
          )}
